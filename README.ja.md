...
Vibe coding が教えてくれたこと:
- コードは速く流れ出る。
- 大抵のコードは誰が書いてもだいたい同じ。
- エージェントが typing を肩代わりできる。
...
元々開発者は後者をやる人でした。1行書く前に — 要件を正確に読み、エッジケースを描き、データモデルをスケッチし、アルゴリズムを選び、失敗の可能性を先に見て、トレードオフを決めて、それからキーボードを叩く。

Vibe coding はこの7ステップを「describe what you want, accept what comes」の1ステップに圧縮しました。時間は減りましたが、消えたステップ1つにつき1ヶ月後の事故が1件、地面に埋まります。

Dive coding はその7ステップを1つずつ戻します。エージェントが typing を肩代わりする席はそのままに、その前にあった *考える* 工程を人間に戻す方式で。エージェントは typing の代わりに *質問* でその思考を引き出します。

| | Vibe coding | Dive coding |
|---|---|---|
| 入力 | 一行の願い | 仕様(だんだん精緻化される) |
...
Dive coding は私たちを vibe-wisher からより慎重な wisher へと戻します。エージェントがより良い答えを出すのではなく、人がより良い質問を投げるよう仕向けることで。
